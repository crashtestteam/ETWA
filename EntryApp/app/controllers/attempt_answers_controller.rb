class AttemptAnswersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @attempt_answer.save
        ...
        format.json { render action: 'show', status: :created, location: @attempt_answer }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to attempt_answers_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def attempt_answer_params
      params.require(:attempt_answer).permit(:answer_id, :attempt_id, :chosen)
    end
end
